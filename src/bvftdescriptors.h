...
Mat get_trans_icp_3_points(const Mat &src_, const Mat &dst_)
{
    ...
    float mean_src_dis = sqrt(temp_src_ptr_x[0] * temp_src_ptr_x[0] + temp_src_ptr_y[0] * temp_src_ptr_y[0]) + sqrt(temp_src_ptr_x[1] * temp_src_ptr_x[1] + temp_src_ptr_y[1] * temp_src_ptr_y[1]) + sqrt(temp_src_ptr_x[2] * temp_src_ptr_x[2] + temp_src_ptr_y[2] * temp_src_ptr_y[2]);
    mean_src_dis /= 3;
    float mean_dst_dis = sqrt(temp_dst_ptr_x[0] * temp_dst_ptr_x[0] + temp_dst_ptr_y[0] * temp_dst_ptr_y[0]) + sqrt(temp_dst_ptr_x[1] * temp_dst_ptr_x[1] + temp_dst_ptr_y[1] * temp_dst_ptr_y[1]) + sqrt(temp_dst_ptr_x[2] * temp_dst_ptr_x[2] + temp_dst_ptr_y[2] * temp_dst_ptr_y[2]);
        ...
        try
        {
            ...
            for (int k = 0; k < used_points; k++)
            {
                for (int l = k + 1; l < used_points; l++)
                {
                    ...
                    if (!is_neighbor)
                        is_neighbor = sum(abs(src_homo.row(ind_vec[k]) - src_homo.row(ind_vec[l])))[0] < 3;
                    if (!is_neighbor)
                        is_neighbor = sum(abs(dst_homo.row(ind_vec[k]) - dst_homo.row(ind_vec[l])))[0] < 3;
                }
            }
            if (is_neighbor)
                continue;
            ...
            cv::reduce(err_temp, err, 0, REDUCE_SUM); // one row
...
Mat matCos(Mat &x)
{
    ...
    Mat y = temp_x - temp_x.mul(x2) / 6 + temp_x.mul(x2).mul(x2) / 120 - temp_x.mul(x2).mul(x2).mul(x2) / 5040;
    return y;
}
...
BVFT detectBVFT(Mat img1)
{
    ...
    for (int k = 0; k < keypoints.size(); k++)
    {
        ...
        Mat patch_true = patch(Rect((patch_size - patch_size_true) / 2, (patch_size - patch_size_true) / 2,
